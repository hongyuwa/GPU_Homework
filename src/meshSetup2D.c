...
mesh2D *meshSetup2D(char *filename, int N){
  ...
  // load reference (r,s) element nodes (quads)
  meshLoadReferenceNodesQuad2D(mesh, N);

  // compute physical (x,y) locations of the element nodes (quads)
  meshPhysicalNodesQuad2D(mesh);

  // compute geometric factors (quad)
  meshGeometricFactorsQuad2D(mesh);

  // compute surface geofacs (quad)
  meshSurfaceGeometricFactorsQuad2D(mesh);

//  // set up halo exchange info for MPI (do before connect face nodes) (quad)
//  meshHaloSetupQuad2D(mesh);
